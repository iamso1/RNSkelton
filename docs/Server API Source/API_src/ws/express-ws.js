 ...
 */
module.exports = function (app, server_s) {
  var server;
  //if(!server) {
  ...
  function addSocketRoute_s(route, middleware, callback) {
    var args = [].splice.call(arguments, 0);
    var wsPath = urlJoin(app.mountpath, route);
  ...
  if (server_s) app.wss = addSocketRoute_s;
  ...
  return {
    ...
    getWsss: function (route) {
		return wssServers[route];
    }
  };
};
